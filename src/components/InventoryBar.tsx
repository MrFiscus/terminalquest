    ...
    >
      ...
      <div className="flex justify-center gap-3">
        {displaySlots.map((slot, idx) => {
          ...
          // Carved-stone alcove: deep inset + subtle interior + (active) torch-glow rim
          const baseAlcove = [
            "inset 0 0 0 2px hsl(var(--stone-slab-edge))",
            "inset 2px 2px 0 hsl(var(--stone-light) / 0.45)",
            "inset -2px -2px 0 hsl(0 0% 0% / 0.5)",
            "inset 0 3px 6px hsl(0 0% 0% / 0.55)",
            "inset 0 -2px 4px hsl(0 0% 0% / 0.4)",
          ].join(", ");
          const activeAlcove = [
            "inset 0 0 0 2px hsl(33 100% 50%)",
          ...
          if (slot.kind === "filled") {
            return (
              <div
                ...
                style={{
                  boxShadow: isActive ? activeAlcove : baseAlcove,
                  filter: "brightness(1.15)",
                }}
                title={slot.item.name}
              >
                <span className="text-2xl drop-shadow-[0_2px_2px_hsl(0_0%_0%/0.85)] drop-shadow-[0_0_6px_hsl(var(--gold)/0.45)]">
                  {slot.item.glyph ?? "▣"}
                </span>
          ...
          return (
            <div
              ...
              style={{ boxShadow: baseAlcove, filter: "brightness(0.95)" }}
            />
          );
        })}
      </div>
